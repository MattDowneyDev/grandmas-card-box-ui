...
export default function App() {
  ...
  const [activeTab, setActiveTab] = useState<NavigationTab>('my-box');

  // Currently inspected recipe for detail modal
  const [selectedRecipe, setSelectedRecipe] = useState<Recipe | null>(null);

  // Modals
  const [faqModalType, setFaqModalType] = useState<'faq' | 'privacy' | 'terms' | null>(null);
  ...
  const handleSaveRecipe = (newRecipeData: Omit<Recipe, 'id' | 'createdAt' | 'isUserUpload'>) => {
  ...
  return (
    <div
      className={`min-h-screen flex flex-col transition-colors duration-150 ${
        theme === 'dark' ? 'bg-[#030712] text-[#dde1ff]' : 'bg-[#dcd9d9] text-[#1b1c1c]'
      }`}
    >
